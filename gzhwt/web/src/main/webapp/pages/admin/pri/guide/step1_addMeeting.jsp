...
$(function(){
	...
	var meetId="${meeting.id}";
	if(meetId==null||meetId==''){
		$("#showProcess").hide();
		//$("#editBase").prev().hide();
		$("#editBase").css("width","90%");
	}else{
		$("#showProcess").show();
		//$("#editBase").prev().show();
		$("#editBase").css("width","75%");
	}
 
	var intro="会议基本信息是对会议所必须的信息进行录入,包括会议名称,会议主题,会议地点,会议开始时间,会议结束时间,会议主办方/承办方等信息";
	$("#intro").html(intro);
...
<div style="width: 75%;float:left;margin-left:5px;margin-right: 20px;" id="editBase">
		<div class="mainbox">
		    <div class="page_title">
				<h3>会议向导 -> 第一步：添加会议基本信息</h3>	
			</div>
			...
			<input type="hidden" name="guideStep" value="step1" />
			<input type="hidden" name="fromStep" value="step2" />
			...
			<fieldset>
		        ...
		        <dl>
		            ...
		            <dd>
		            	<input class="input1"  type="text" style="width:80%" class="half" id="meeting_name" name="meeting.name"  value="${fn:escapeXml(meeting.name)}" maxlength="40"/>
		        ...
		        <dl>
		            ...
		            <dd>
		       	<textarea style="width:80%" class="medium" id="meeting.topic" name="meeting.topic" maxlength="200">${fn:escapeXml(meeting.topic)}</textarea>
            	<small>请限制在200字以内,您还可以输入<span id="meeting_topic_remain">200</span>字</small>
		            <%-- 
		            	<input class="input1"  type="text" style="width:40%" class="half" id="meeting.topic" name="meeting.topic"  value="${fn:escapeXml(meeting.topic)}" maxlength="40"/>
		            	<small>请限制在40字以内,您还可以输入<span id="meeting_topic_remain">40</span>字</small>--%>
		         </dd>
		        </dl>
		        <dl>
		        	<dt>
		            ...
		            <dd>
		            	<select class="input1" id="province" name="meeting.province" style="width:80px;" class="valid"></select>
						<select class="input1" id="city" name="meeting.city" style="width:80px;" ></select>
		            	<input class="input1" style="width:53%"  type="text" id="meeting.location" name="meeting.location"  value="${fn:escapeXml(meeting.location)}" maxlength="30"/>
		        ...
		        <dl style="float:left;width:40%">
		        	<dt>
		            	 <label for="title"> <font color="red">*</font>会议时间 开始：</label>
		            </dt>
		            
		            <dd style="width:40%">
		            	<input class="input1" id="startTime" type="text" name="meeting.startTime"  value="${fn:substring(meeting.startTime,0,16)}"  class="Wdate" onfocus="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd HH:mm',startDate:'%y-%M-%d %H:00:00',maxDate:'#F{$dp.$D(\'endTime\')||\'2019-12-31 08:00\'}' })" readonly="readonly"/>
		            </dd>
		        </dl>
		        
		        <dl style="float:left;width:42%">
		        	<dt >
		            	<label for="title"><font color="red">* </font>结束：</label>
		            </dt>
		            <dd style="width:42%">
		            	<input class="input1" id="endTime" type="text" name="meeting.endTime"  value="${fn:substring(meeting.endTime,0,16)}"  class="Wdate" onfocus="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd HH:mm',startDate:'%y-%M-%d %H:00:00',minDate:'#F{$dp.$D(\'startTime\')||\'2019-12-31 18:00\'}',maxDate:'2019-12-31 18:00'})" readonly="readonly"/>
		            </dd>
		         </dl>
             ...
             <c:if test="${SESSION_ADMIN_USER.role.id eq 1 }">
		        ...
		        <dl>
		            ...
		            <dd>
			                <c:if test="${empty meeting.id  }">
				                <c:if test="${SESSION_ADMIN_USER.role.id eq 1 }">
									<input class="input1" style="width:20%" id="freeSmsNum" type="text" name="meeting.freeSmsNum"  value="${meeting.freeSmsNum}" maxlength="8" />	
								</c:if>
								<c:if test="${SESSION_ADMIN_USER.role.id ne 1 }">
			                    	<input type="hidden" id="freeSmsNum" name="meeting.freeSmsNum"  value="0" />
			                ...
			                <c:if test="${not empty meeting.id}">
				                ...
				                <c:if test="${SESSION_ADMIN_USER.role.id ne 1 }">
			                    	<input class="input1" type="hidden" id="freeSmsNum" name="meeting.freeSmsNum"  value="${meeting.freeSmsNum}" />
		        ...
		        <dl >
		            ...
		            <dd>
						<input class="input1" type="text" id="host" name="meeting.host"  style="width:80%"  value="${meeting.host}" maxlength="50" />
		            </dd>
		        </dl>
		          ...
		          <dl>
		            ...
		            <dd>
		            	<textarea class="input1"  style="width:80%"  id="meeting.comments" name="meeting.comments" maxlength="200">${fn:escapeXml(meeting.comments)}</textarea>
        ...
        <dl class="org_dl">
            ...
            <dd>
				...
				<input type="text" id="organizer" name="orgList[0].organizer"  style="width:40%" value="" maxlength="50" />
        ...
        <c:if test="${not empty meeting.organizer }">
        	<c:forEach var="orgzer" items="${wd:orgsToList(meeting.organizer) }" varStatus="status">
		        ...
		        --%>
		        	<input id="state_valid" name="meeting.state" value="1" type="hidden" />
		    ...
		    <div class="page_form_sub">
		        <a href="${ctx}/admin/pri/meeting/guide_begin.action" id="cancelBtn" class="btn_common btn_true">返&nbsp;&nbsp;回</a>
		        <a href="##" onclick="save();" id="submitBtn" class="btn_common btn_true">下一步</a>    
		    </div>
...
$().ready(function() {
	...
	$("#meeting\\.topic").calcWordNum({maxNumber:40,targetid:"meeting_topic_remain"});
	...
	$("#addMeetingForm").validate({
		...
		function save(){
			var mid = "${meeting.id}";
			if(mid == "" && ! confirm("注意：在向导完成第一步之后，会议信息已经创建，如果需要撤消，可以通过会议列表来删除所创建的新会议信息。\n\n确定进入下一步吗？")) {
				return false;
			}
		...
		function callback(data){
			...
			//修改不需要提示信息
			if(data.fromStep != "step2") {
				alert(data.retmsg);  
			}
			
			if(data.retcode=="0"){
				//判断是否是会议导向建会
				if(data.guideStep == "step1") {
					window.location.href = "${ctx}/admin/pri/meeting/guide_step2.action?guideStep=step2&meeting.id=" + data.retdata.id;
					return;
				}
