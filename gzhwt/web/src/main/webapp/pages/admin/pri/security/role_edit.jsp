...
<body>
	...
	<div title="角色信息编辑" selected="true"  style="padding:10px;"></div>
	...
	<form id="addFrm"  method="post">
	    ...
	    <fieldset>
	        ...
	        <dl style="width:48%;float:left;">
	            ...
	            <dd style="width:40%">
	            	<input type="text"  class="half" id="roleName" name="roleName" value="${role.roleName}" tabindex="2" maxlength="30"></input>	            	
	            </dd>
	        </dl>
	        <dl style="width:48%;float:left;">
	        	<dt>
	            ...
	            <dd  style="width:40%" >
	            	<input type="text"  class="half" id="roleDescription" name="roleDescription" tabindex="2" maxlength="30" value="${role.roleDescription}"></input>	 
	            </dd>
	        </dl>	
	      ...
	      <c:if test="${not empty unitViewList }">
	      		<c:forEach var="securityUnitView" items="${unitViewList}" varStatus="status">
	      	          <input id="" name="unitId" type="checkbox" <c:if test="${fn:contains(unitIds,securityUnitView.securityParentUnit.id)}"> checked="checked" </c:if>  value="${securityUnitView.securityParentUnit.id}"/>${securityUnitView.securityParentUnit.unitName}<br />
	      	            <c:if test="${not empty securityUnitView.sonUnitList}">
	      	          	<c:forEach var="sonUnit" items="${securityUnitView.sonUnitList}" varStatus="status">
	      	          	   &nbsp;&nbsp;&nbsp; <input name="unitId"  type="checkbox"  <c:if test="${fn:contains(unitIds,sonUnit.id)}"> checked="checked" </c:if> value="${sonUnit.id}"/>${sonUnit.unitName} <br />
	      	          	</c:forEach>
	      	          </c:if>
	      	 </c:forEach>
	      </c:if>
