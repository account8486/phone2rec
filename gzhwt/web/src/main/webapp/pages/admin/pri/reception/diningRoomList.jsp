		...
		<div title="餐厅管理" selected="true" style="padding:10px;">
			...
			<form id="mainForm" action="${ctx}/admin/pri/dr/list.action">
				...
				<table width="98%">
			      <tr>
			        <th>餐厅名称：</th>
			        <td><input type="text" id="diningRoomName" name="diningRoom.name"
		                                   value="${diningRoom.name }"/></td>
			...
			<div class="page_tools page_toolbar">
				<a href="#" id="addBtn" class="btn_common btn_false" style="margin-left:5px;">添加餐厅</a>
		                    ...
		                    <c:when test="${not empty pager.pageRecords}">
		                        <c:forEach var="record" items="${pager.pageRecords}" varStatus="status">
		                            <tr <c:if test="${status.count % 2 eq 0}"> class="even"</c:if>>
		                                ...
		                                <td>${record.name }</td>
		                                <td width="40px"><a href="${ctx}/admin/pri/dr/goUpdate.action?id=${record.id}&meetingId=${meetingId}">编辑</a>
		                                </td>
		                               	<td width="40px"><a href="#" onclick="delDiningRoom(${record.id})">删除</a></td>
		                            </tr>
		                        </c:forEach>
		                    </c:when>
		                    <c:otherwise>
		                        <tr class="datarow">
		                            <td colspan="5" align="center"> 没有餐厅信息.</td>
		                        </tr>
		                    </c:otherwise>
		                	</c:choose>
			            </tr>
...
<script type="text/javascript">
	function delDiningRoom(id)
	{
		if(confirm("您确认要删除该餐厅吗？"))
		{
			window.location.href = "${ctx}/admin/pri/dr/del.action?id="+ id + "&meetingId=" + $("#meetingId").val();
		}
	}
	...
	$(document).ready(function(){
		...
		$("#addBtn").click(function(){
			window.location.href = "${ctx}/admin/pri/dr/goAdd.action?meetingId=" + $("#meetingId").val();
		});
    ...
    jQuery(document).keypress(function(e){
    	if(e.which == 13 ) {
    		...
    		switch(act){
    			case 'diningRoomName':$('#queryForList').click();break;
