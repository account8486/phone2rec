	...
	<form id="mainForm" action="${ctx}/admin/pri/reception/specialty_addLocalSpecialty.action" method="post" enctype="multipart/form-data">
	    ...
	    <fieldset>
	        ...
	        <dl id="dlImage">
	            ...
	            <dd>
	            	<c:if test="${actionType eq 'editLocalSpecialty' and not empty localSpecialty.image}">
...
<script type="text/javascript">
	...
	function ajaxSearchCallback(data) {
		...
		if(data.specialtyList.length == 0) {
			dlSearchResult.hide();
			return;
		}
		
		for(var i = 0; i < data.specialtyList.length; i++) {
			var li = "<li class='result'>" + data.specialtyList[i][1] 
				+ "<a href='javascript:chooseResult(" + data.specialtyList[i][0]
				+ ")'>选择</a></li>";
	...
	function validate() {
	    ...
	    if(actionType == "chooseLocalSpecialty") {
	    	return true;
	    }
	    
	    var name = $("#name").val();
	    if(isEmpty(name)) {
	    	$("#tip_name").html("请输入土特产名称。");
	        $("#tip_name").show();
	        result = false;
	    }
