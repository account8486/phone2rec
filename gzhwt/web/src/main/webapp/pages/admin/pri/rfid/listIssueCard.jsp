...
<body>
			...
			<table>
		      <tr>
		        ...
		        <td><a href="#" id="queryForList" class="btn_common btn_true">搜 索</a>
		        	<a href="javascript:delAll()" class="btn_common btn_true">全部收回</a>
	...
	<table class="page_datalist">
	    	<thead>
	        	<tr>
	            	<th width="2px"><input type="checkbox" id="chbSelectAll" onclick="doSelectAll()"/></th>
	            	<th width="10%">参会人</th>
                    ...
                    <c:when test="${not empty pager.pageRecords}">
                        <c:forEach var="issue" items="${pager.pageRecords}" varStatus="status">
                            <tr <c:if test="${status.count % 2 eq 0}"> class="even"</c:if>>
                                <td ><input type="checkbox" name="chbIssueId" value="${issue.id }"/></td>
                                ...
                                <td>
                                 	<a href="#" onclick="delCard(${issue.id},'${issue.owner.name }');">收回</a>
                                 	<%-- 
...
<script type="text/javascript">
	...
	// 删除
	function delCard(id,username)
	{
		if(confirm("确认要回收" + username + "的RFID卡牌吗？"))
		{
	...
	function delAll() {
		var ary = $("input[name='chbIssueId']:checked");
		if(ary.length == 0) {
			alert("没有选择要回收的RFID卡牌。");
			return;
		}
		...
		window.location.href = "${ctx}/admin/pri/meeting/card_batchDeleteIssue.action?meetingId=${meetingId}&issueIds="+ ids;
	}
	
	function doSelectAll() {
		$("input[name='chbIssueId']").prop("checked", $("#chbSelectAll").prop("checked"));
	}
