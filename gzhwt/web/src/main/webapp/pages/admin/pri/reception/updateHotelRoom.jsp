...
<body>
	...
	<div class="page_form">
	<form id="mainForm" action="${ctx}/admin/pri/room/update.action" method="post" enctype="multipart/form-data"}>
	    <input type="hidden" id="meetingId" name="meetingId" value="${meetingId}"/>
	    <input type="hidden" id="hotelRoomId" name="hotelRoom.id" value="${hotelRoom.id}"/>
	    <fieldset>
	        <legend></legend>
	        <dl>
	        	<dt>
	            ...
	            <dd>
	            	<select class="inp_1" id="hotelRoomTypeId" name="hotelRoom.hotelRoomType.id">
						<c:forEach var="record" items="${hotelRoomTypeList}" varStatus="status">
							<option value="${record.id }" <c:if test="${record.id eq hotelRoom.hotelRoomType.id }"> selected </c:if>>${record.hotel.name }_${record.name }</option>
						</c:forEach>
					</select>
	        ...
	        <dl>
	            ...
	            <dd>
	            	<input class="half" type="text" id="roomNo" name="hotelRoom.roomNo" value="${hotelRoom.roomNo}" maxlength="20"/>
