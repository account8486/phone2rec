...
<head>
	...
	<script type="text/javascript">
		...
		function validateAdd() {
	        ...
	        if (role == 1 && org != 0)
	        {
	        	$("#tip_role").html("对不起，只有站点管理组织下的用户才能是超级管理员。");
	            $("#tip_role").show();
	            return false;
	        }
		...
		$(document).ready(function(){
			...
			$("#securityRoleId").val(${user.securityRoleId});
			
			$("#role").change(function(){
				// 会议管理员，并且当前用户拥有管理组织机构的权限。
				if($(this).val() == 3 && ${ SESSION_ADMIN_USER.canOrg} == 1)
				{
					$("#canOrg").show();	
				} else {
					$("#canOrg").hide();
	...
	<form id="updateUserForm" action="${ctx}/admin/pri/user/update.action">
	    ...
	    <fieldset>
	        ...
	        <dl>
	            ...
	            <dd>
	            	<select style="width:16%;" class="inp_1" id="role" name="user.role.id" tabindex="4">
						<c:forEach var="role" items="${roleList}" varStatus="status">
							<option value="${role.id }" <c:if test="${role.id eq user.role.id }"> selected </c:if>>${role.roleName }</option>
						</c:forEach>
					</select>
					<font id="tip_role" style="line-height:35px" color="red"></font>
					<select style="width:16%;" class="inp_1" id="canOrg" name="user.canOrg" tabindex="5" style="display:none;">
						<option value="1" <c:if test="${user.canOrg eq 1 }"> selected </c:if>>可以管理组织机构</option>
						<option value="0" <c:if test="${user.canOrg eq 0 }"> selected </c:if>>不可以管理组织机构</option>
					</select>
	            </dd>
	        </dl>
	          ...
	          <dl>
	        	<dt>
	            	<label for="title"><font color="red">* </font>菜单角色：</label>
	            </dt>
	            <dd>
	            	<select class="inp_1" id="securityRoleId" name="user.securityRoleId" tabindex="5" style="width:16%;">
	    ...
	    <fieldset>
	        ...
	        <dl>
	            ...
	            <dd>
	            	<select class="inp_1" id="user.gender" name="user.gender" tabindex="6" style="width:16%;">
	        ...
	        <dl>
	            ...
	            <dd>
	            	<select class="inp_1" id="user.state" name="user.state" tabindex="7" style="width:16%;">
