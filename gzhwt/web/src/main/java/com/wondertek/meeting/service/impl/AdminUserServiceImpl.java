...
//		userLoginLogDao.add(adminUserLog);
	...
	public Pager<AdminUser> findAdminUserPager(AdminUser adminUser, List<Long> childOrgIdList, Long selfOrgId,
		...
		// 能查看本级低角色用户及下级组织所有用户
		String hql = "from AdminUser u where ( 1=0 ";

		if (roleList != null && roleList.size() > 0) {
			hql += "or (u.org.id = :selfOrgId and u.role.id in(:roleList)) "; // 本级低角色用户
		}

		if (childOrgIdList != null && childOrgIdList.size() > 0) {
			hql += "or u.org.id in(:childOrgIdList) "; // 下级组织所有用户
		}

		hql += ") and u.mobile like '%'||:mobile||'%' and u.name like '%'||:name||'%' and u.state != -1 order by modifyTime desc";

		Map<String, Object> properties = new HashMap<String, Object>();
		properties.put("mobile", StringUtil.null2Str(adminUser.getMobile()));
		properties.put("name", StringUtil.null2Str(adminUser.getName()));
		properties.put("childOrgIdList", childOrgIdList);
		properties.put("selfOrgId", selfOrgId);
		properties.put("roleList", roleList);
		properties.put("selfUserId", selfUserId);
	 ...
	 * @return
	 */
	public List<AdminUser> getAdminUsersByOrg(Long orgId, Long roleId) {
		return adminUserDao.getAdminUsersByOrg(orgId, roleId);
	}
