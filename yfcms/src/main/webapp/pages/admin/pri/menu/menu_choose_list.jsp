                    ...
                    <c:when test="${not empty webMenuList}">
                        <c:forEach var="clientMenu" items="${webMenuList}" varStatus="status">
                            <tr <c:if test="${status.count % 2 ne 0}"> class="odd"</c:if>>
                                ...
                                <!-- 
                                <c:if test="${clientMenu.name eq '会议介绍'}">
                                ...
                                <input type="checkbox"  name="webMenuCk" id="webMenuCk${status.count}" value="${clientMenu.id}" <c:if test="${clientMenu.name eq '会议介绍'}"></c:if>  <c:if test="${fn:contains(choosedWebMenuStr,clientMenu.id)}"> checked="checked" </c:if>  >
                                ${clientMenu.name}</td>  
